...
export function MangaCard({ manga }: MangaCardProps) {
  ...
  return (
    <Card className="overflow-hidden h-full rounded-lg border-0 bg-transparent">
      {/* FIX: replaced <a> with Next.js <Link> for client-side navigation */}
      <Link
          ...
          {coverKey ? (
            <div className="relative w-full h-full">
              ...
              {manga.md_comics?.status && (
                <div
                  className={`absolute top-2 right-2 px-2 py-1 rounded text-xs font-medium text-white ${getStatusColor(manga.md_comics.status)}`}
                >
