...
export default function MangaReader() {
  ...
  // FIX: Cache the manga hid so "Load More" doesn't re-fetch comic info every time.
  const mangaHidRef = useRef<string | null>(null);

  const fetchChapters = async (hid: string, pageNum: number, order: 0 | 1) => {
    ...
    return chaptersResponse.json() as Promise<ChapterResponse>;
  };

  // Initial load: fetch manga info + first page of chapters
  const fetchMangaData = async () => {
    ...
    try {
      ...
      const hid = mangaData.comic.hid;
      mangaHidRef.current = hid;
      setMangaInfo(mangaData.comic);

      const chaptersData = await fetchChapters(hid, 1, chapterOrder);
  ...
  // Load more: only fetch chapters, reuse cached hid
  const handleLoadMore = async (e: { preventDefault: () => void }) => {
    e.preventDefault();
    if (!mangaHidRef.current) return;
    const nextPage = page + 1;
    setPage(nextPage);
    setLoading(true);
    try {
      const chaptersData = await fetchChapters(mangaHidRef.current, nextPage, chapterOrder);
  ...
  const getCoverImage = (covers: MangaCover[]) => {
    if (!covers || covers.length === 0) return "/placeholder.svg";
    return `/api/image/${covers[0].b2key}`;
  };

  const getGenres = (genreData: MangaGenre[]) => {
    if (!genreData) return [];
    return genreData.map((item) => ({
      name: typeof item.md_genres?.name === "string" ? item.md_genres.name : "Unknown",
    }));
  };

  const handleOrderChange = () => {
    setChapterOrder((prev) => (prev === 0 ? 1 : 0));
    setPage(1);
    setChapters([]);
  };
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-6 md:mb-8">
        ...
        >
          <div className="relative aspect-[629/911] w-full rounded-lg overflow-hidden shadow-lg">
            <Image
              src={getCoverImage(mangaInfo.md_covers)}
              alt={mangaInfo.title}
              fill
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6 text-center md:text-left">
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
              ...
              <span className="text-xs sm:text-sm">
                <span className="font-medium">Status:</span> {getStatusText(mangaInfo.status)}
              </span>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
              <BookMarked className="h-4 w-4 sm:h-5 sm:w-5 text-muted-foreground mx-auto md:mx-0" />
              <span className="text-xs sm:text-sm">
                <span className="font-medium">Chapters:</span> {mangaInfo.chapter_count}
              </span>
            </div>
            <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
      ...
      <div>
        ...
        <div className="mb-4">
          <div className="flex flex-col sm:flex-row gap-3 mb-3">
            ...
            <div className="flex-none">
              ...
              >
                {chapterOrder === 0 ? (
                  <>Newest First <ChevronRight className="h-4 w-4 rotate-90" /></>
                ) : (
                  <>Oldest First <ChevronRight className="h-4 w-4 -rotate-90" /></>
                )}
              </Button>
            </div>
          </div>
          {(searchTerm || searchGroup) && (
            <div className="flex justify-between items-center text-sm">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => { setSearchTerm(""); setSearchGroup(""); }}
              >
                Clear Filters
              </Button>
            </div>
                ...
                <CardContent className="p-3 sm:p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-3">
                      <div className="font-bold text-base sm:text-lg">Ch. {chapter.chap}</div>
                      ...
                      <div className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground">
                        ...
                        <span className="sm:hidden">{formatDateShort(chapter.publish_at)}</span>
                        <ThumbsUp className="h-3 w-3 sm:h-4 sm:w-4 ml-2 sm:ml-0" />
                        {chapter.up_count}
                      </div>
                    </div>
                    <div className="flex items-center gap-1 sm:gap-2">
                      <Badge variant="outline" className="text-xs px-1.5 py-0 sm:px-2 sm:py-0.5">
        ...
        {filteredChapters.length === 0 && (searchTerm || searchGroup) && (
          <div className="py-8 text-center">
            <p className="text-muted-foreground">No chapters match your search criteria.</p>
          </div>
        )}
...
function MangaInfoSkeleton() {
  return (
    <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-6 md:mb-8">
      <div className="w-full md:w-1/3 lg:w-1/4 flex-shrink-0 mx-auto md:mx-0" style={{ maxWidth: "250px" }}>
        <Skeleton className="aspect-[629/911] w-full rounded-lg" />
      </div>
      <div className="flex-1">
        <Skeleton className="h-8 sm:h-10 w-3/4 mx-auto md:mx-0 mb-4" />
        <div className="flex flex-wrap justify-center md:justify-start gap-1.5 sm:gap-2 mb-4">
          {[1, 2, 3, 4, 5].map((i) => <Skeleton key={i} className="h-5 sm:h-6 w-16 sm:w-20" />)}
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4 mb-4 sm:mb-6">
          {[1, 2, 3].map((i) => <Skeleton key={i} className="h-5 sm:h-6 w-full" />)}
        </div>
        <div className="mb-4 sm:mb-6">
